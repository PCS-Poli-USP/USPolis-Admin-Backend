...
def break_class_into_events(class_obj, preference_building):
    event_list = []
    buildings_tb = database["building"]
    ...
    if isinstance(class_obj["start_time"], list):
        for i in range(len(class_obj["start_time"])):
            ...
            if len(class_obj["professors"]) > i:
                ...
                if len(class_obj["professors"]) > 0:
                    event["professors"] = [class_obj["professors"][0]]
                else:
                    event["professors"] = ["Desconhecido"]
            ...
            event["start_period"] = datetime.strptime(
                class_obj["start_period"], "%d/%m/%Y"
            ).strftime("%Y-%m-%d")
            event["end_period"] = datetime.strptime(
                class_obj["end_period"], "%d/%m/%Y"
            ).strftime("%Y-%m-%d")
            ...
            event["preferences"] = {
                "building_id": preference_building,
