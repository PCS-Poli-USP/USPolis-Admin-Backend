...
def get_jupiter_class_infos(subject_code):
    URL = (
        f"https://uspdigital.usp.br/jupiterweb/obterTurma?nomdis=&sgldis={subject_code}"
    )
    ...
    table3_filter = {"tag": "span", "attrs": {"class": "txt_arial_8pt_black"}}
    ...
    for c_info in class_div:
        ...
        info3 = parse_table(
            info_table[2], orientation="vert", labels=table3_names, filter=table3_filter
        )
        ...
        out.append(
            {
                ...
                "inscritos": list(
                    [int(x) if x.isdigit() else 0 for x in info3["inscritos"]]
                ),
                "pendentes": list(
                    [int(x) if x.isdigit() else 0 for x in info3["pendentes"]]
                ),
            }
        )
