...
class Classroom(Document):
    ...
    @classmethod
    async def by_building_and_classroom(cls, building_id: str, classroom_name: str) -> Self:
        classroom = await cls.find_one({"building.$id": ObjectId(building_id), "name": classroom_name})
        if classroom is None:
            raise ClassroomInBuildingNotFound(classroom_name, building_id)
        return classroom

    @classmethod
    async def check_classroom_name_exists(cls, building_id: str, classroom_name: str) -> bool:
        """Check if a classroom name exist in a building"""
        return await cls.find_one({"building.$id": ObjectId(building_id), "name": classroom_name}) is not None

    @classmethod
    async def check_new_classroom_name_exists(cls, building_id: str, classroom_id: str, new_name: str) -> bool:
