...
class ForumRepository:
    ...
    @staticmethod
    def get_post_like_reaction(
        mobile_user_id: int | None, post_id: int, session: Session
    ) -> bool:
    ...
    @staticmethod
    def get_all_posts(
        *, subject_id: int, filter_tags: list[int] | None, session: Session
    ) -> list[ForumPost]:
        ...
        statement = (
            ...
            .where(
                ...
                col(ForumPost.reply_of_post_id) is None,
                col(ForumPost.enabled),
            )
    ...
    def get_post_by_id(*, post_id: int, session: Session) -> ForumPost:
        statement = select(ForumPost).where(
            col(ForumPost.id) == post_id, col(ForumPost.enabled)
        )
        post = session.exec(statement).one()
        return post

    @staticmethod
    def disable_post(*, post_id: int, session: Session) -> None:
    ...
    def update_forum_report_count(
        *, post_id: int, mobile_user_id: int, session: Session
        ...
        if reported_post is None:
            ...
            reports_count = (
                len(list(session.exec(reports_statement).all())) + 1
            )  # Adds the current report to counter
            post.report_count = reports_count

            # Create new report for this post and user
            new_reported_post = ForumPostReportLink(
                forum_post_id=post_id, mobile_user_id=mobile_user_id
            )
    ...
    @staticmethod
    def create_reply(*, input: ForumPost, session: Session) -> ForumPost:
        try:
            # Gets, and counts, all replies from this post (except this new one)
            replies_statement = select(ForumPost).where(
                col(ForumPost.reply_of_post_id) == input.reply_of_post_id
            )
            replies_count = (
                len(list(session.exec(replies_statement).all())) + 1
            )  # Adds the current reply to counter
            post = session.exec(
                select(ForumPost).where(col(ForumPost.id) == input.reply_of_post_id)
            ).first()
    ...
    @staticmethod
    def get_all_replies(*, post_id: int, session: Session) -> list[ForumPost]:
        statement = (
            select(ForumPost)
            .where(
    ...
    def change_forum_post_like(
        *, post_id: int, mobile_user_id: int, like_state: bool, session: Session
        ...
        if liked_post is None:
            new_react = ForumPostReactsLink(
                forum_post_id=post_id,
                mobile_user_id=mobile_user_id,
                post_like=like_state,
            )
        ...
        likes_statement = select(ForumPostReactsLink).where(
            col(ForumPostReactsLink.forum_post_id) == post_id,
            col(ForumPostReactsLink.post_like),
        )
