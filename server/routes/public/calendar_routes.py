...
from server.models.http.requests.calendar_request_models import CalendarRegister, CalendarUpdate
...
async def create_calendar(
    ...
    calendar = CalendarRepository.create(
        creator=user, input=input, session=session)
    return CalendarResponse.from_calendar(calendar)


@router.put("/{calendar_id}")
async def update_calendar(calendar_id: int, calendar_input: CalendarUpdate, user: UserDep, session: SessionDep) -> CalendarResponse:
...
async def delete_calendar(calendar_id: int, user: UserDep, session: SessionDep) -> Response:
