...
def simple_cache(expire_seconds: int = 60) -> Callable[[Callable[..., T]], Callable[..., T]]:
        ...
        if inspect.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args: Any, **kwargs: Any) -> T:
                ...
                if cache_key in _cache_store:
                    ...
                    if BrazilDatetime.now_utc() - cached_time < timedelta(seconds=expire_seconds):
        ...
        if inspect.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args: Any, **kwargs: Any) -> T:
                ...
                if interval_param:
                    ...
                    if interval and hasattr(interval, 'start') and hasattr(interval, 'end'):
                        if interval.start and interval.end:
                            start_date = datetime.combine(interval.start, datetime.min.time())
                            end_date = datetime.combine(interval.end, datetime.min.time())
                else:
...
# Funções úteis para gerenciamento do cache
def clear_cache() -> int:
...
def clear_expired_cache() -> int:
    ...
    expired_keys = [
        key for key, (_, cached_time) in _cache_store.items()
