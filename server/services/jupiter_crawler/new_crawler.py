...
class JupiterCrawler:
    ...
    @staticmethod
    async def crawl_subject_static(subject_code: str) -> CrawledSubject:
        crawler = JupiterCrawler()
        return await crawler.crawl_subject(subject_code)

    async def crawl_subject(self, subject_code: str) -> CrawledSubject:
        self.__subject_code = subject_code
        self.__soup = await self.__build_soap(self.__build_url())
    ...
    def __build_url(self) -> str:
        return BASE_URL + self.__subject_code

    async def __build_soap(self, url: str) -> BeautifulSoup:
    ...
    def __extract_classes_info(self) -> list[CrawledClass]:
        ...
        for class_soup in classes_soups:
...
if __name__ == "__main__":
    ...
    async def main() -> None:
        ...
        crawler = JupiterCrawler()
        subject = await crawler.crawl_subject(subject_code)
