...
class OccupanceReportsService:
    ...
    def get_occupance_reports(session, building_id: int):
        ...
        classes = ClassRepository.get_all_on_classrooms(classroom_ids = classroom_ids, session=session, interval = interval)

        #(classroom_id, week_day, start_time, end_time) é a chave - list é uma lista de classes (valor associado a chave)
        schedule_map: dict[tuple[int, WeekDay | None, time, time], list] = {} #dicionario vazio - tuple (lista imutável)

        for class_ in classes: 
            schedules = ScheduleRepository.get_all_on_class(class_ = class_, session = session) # pegar as agendas de uma sala 
            
            for schedule in schedules:
                if not schedule.classroom_id or not schedule.start_date or not schedule.end_date: #são NOne
                    continue # pula para o próximo schedule 

                if (schedule.recurrence in [Recurrence.WEEKLY, Recurrence.DAILY] ):
                            
                    key = (schedule.classroom_id, schedule.week_day, schedule.start_time, schedule.end_time)
                    schedule_map.setdefault(key, []).append(class_) #se ainda não existe, cria; se existe, adiciona class_ a essa chave
        ...
        for (classroom_id, week_day, start, end), class_list in schedule_map.items():
            ...
            if classroom.capacity > 0:
                ...
                occupance_reports.append({
                    ...
                    "classes": [f"{c.subject.code} - {c.subject.name} ({c.code})" if c.subject else str(c.id) for c in class_list],
                    "start_time": start,
                    "end_time": end,
